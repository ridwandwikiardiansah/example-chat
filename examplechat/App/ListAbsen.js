...
const ListAbsen = ({navigation}) => {
    ...
    const chat = [
        ...
        const getListChat = async () => {
           ...
           try {
            const getList =  await axios.post('http://103.75.26.78:8880/savehr/ws.getlistabsen.php',body)
               ...
               <ScrollView showsVerticalScrollIndicator={false}>
                   {
                       map(dataChat, (i,index) => (
                        <TouchableOpacity style={styles.card} key={index} onPress={()=>navigation.navigate('DetailAbsen', {data:i})}>
                            <Text style={styles.username}>{i.username}</Text>
                            <Text numberOfLines={1}>{i.lokasi}</Text>
